...
class Profile:
    ...
    def __init__(self, dataset=None, profile=None):
        ...
        if type(profile) != pandas.core.frame.DataFrame:

            # Initializing a new DF for the new profile with a new index or user number
            try:
                self.profile = pandas.DataFrame(index=[self.dataset.index[-1] + 1])

            # If starting from an empty DF
            except:
                self.profile = pandas.DataFrame(index=[0])
        ...
        # A combined DF containing both the original data and the new profile
        # , will be N/A until add_profile_to_dataset() is used
    ...
    def vector_text(self):
        ...
        self.vectorized_text = pandas.DataFrame(vect_words.toarray(),
    ...
    def scale_profile(self, exclude=['Bios']):
        ...
        self.scaled_profile = pandas.DataFrame(index=self.profile.index)
    ...
    def format_profile(self):
        """
        Uses both scaling and vectoring to format the profile DF
        """

        try:
            # If the attributes have already been instantiated by the methods before
            self.formatted_profile = pandas.concat([self.scaled_profile, self.vectorized_text], axis=1)

        except Exception as ex:
            print(ex)
            # If not, run the methods here
            self.formatted_profile = pandas.concat([self.scale_profile(), self.vect_text()], axis=1)
